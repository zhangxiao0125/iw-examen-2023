...
const UserProfile = ({ user }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      const housings = await fetch(
        `http://${process.env.URL}/api/housings/owner/${user.username}`
  ...
  const deleteUser = async () => {
    await fetch(`http://${process.env.URL}/api/users/${user._id}`, {
      method: "DELETE",
  ...
  function property(house) {
    ...
    myImage
      .resize(thumbnail().width(500).height(300))
      ...
      <div className="flex flex-col w-full h-full items-center justify-center space-y-4 py-10">
          ...
          <div className="flex flex-col items-center pb-4">
            ...
            <div className="flex flex-row justifiy-center space-x-2">
              ...
              <p className="text-base text-gray-400">
                {propertiesList.length} viviendas
              </p>
            </div>
          </div>
            ...
            {session ? (
              session.user.email === user.email ? (
                <Button className="bg-blue-600 hover:bg-blue-800">
                  <Link href={`edit-user/?id=${user._id}`} passHref>
                    Editar perfil
                  </Link>
                </Button>
              ) : null
        ...
        <div className="flex flex-col w-full space-y-4">
          <div className="flex flex-row justify-start space-x-4 p-6 md:pl-48">
            <p className="text-2xl font-medium">Aparcamientos en propiedad</p>
          ...
          <div className="overflow-auto hover:overflow-x-scroll flex flex-row justify-start space-x-2 px-6 md:pl-48 pb-4">
            {propertiesList.length > 0 &&
              propertiesList.map((house, i) => {
                return property(house);
              })}
            {propertiesList.length === 0 && (
              <p className="flex flex-col text-lg text-center text-gray-400 bg-gray-50 w-full justify-center items-center py-4 px-6 md:mr-48 rounded-lg">
                Aquí aparecerán los aparcamientos creados por este usuario.
              </p>
            )}
          </div>
        </div>
...
export async function getServerSideProps(ctx) {
  ...
  const user = await fetch(`http://${process.env.URL}/api/users/${id}`).then(
