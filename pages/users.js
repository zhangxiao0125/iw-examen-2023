...
const Users = ({ users, loggedUser }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (event.target.age1.value !== "" && event.target.age2.value !== "") {
      router.push(
        `/users?age1=${event.target.age1.value}&age2=${event.target.age2.value}`
      );
    ...
    <>
      ...
      <div className="flex flex-col pt-10 items-center">
        <Dropdown label="Filtro" inline={true}>
          ...
          <Dropdown.Item onClick={filterUsername}>Apodo</Dropdown.Item>
          <Dropdown.Item onClick={filterAges}>Edades</Dropdown.Item>
        </Dropdown>

        {JSON.stringify(loggedUser, 2)}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex-col flex items-center">
            <Button
              id="submit"
              type="submit"
              className="my-4 bg-blue-600 hover:bg-blue-800"
              style={{ display: "none" }}
            >
              Buscar
            </Button>
                ...
                return (
                  <li className="py-3 sm:py-4">
                    <div className="flex items-center space-x-4">
                      ...
                      <Link href={`user/?id=${user._id}`} passHref>
...
export async function getServerSideProps(ctx) {
  ...
  if (ctx.query.age1 !== undefined && ctx.query.age2 !== undefined) {
    ...
    const users = await fetch(
      `http://${process.env.URL}/api/users/ages/${age1}/${age2}`
    ).then((response) => response.json());
  ...
  if (ctx.query.username !== undefined) {
    ...
    const users = await fetch(
      `http://${process.env.URL}/api/users/username/${username}`
    ).then((response) => response.json());
  ...
  const users = await fetch(`http://${process.env.URL}/api/users`).then(
